            ...
            <div className="p-6 border-b border-[#E2E8F0]">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between">
                    ...
                    <div className="flex items-center space-x-4">
                        {/* Chart Type Toggle */}
                        <div className="flex items-center space-x-1 bg-[#F1F5F9] rounded-md p-1">
                            {chartTypes?.map((type) => (
                                <button
                                    key={type?.id}
                                    onClick={() => setActiveChart(type?.id)}
                                    className={`flex items-center space-x-1 px-3 py-1 rounded text-sm font-medium transition-all duration-200 ${activeChart === type?.id
                                        ? 'bg-[#FFFFFF] text-text-primary shadow-md'
                                        : 'text-[#738192] hover:text-text-primary'
                                        }`}
                                >
                        ...
                        <select
                            ...
                            className="border border-[#E2E8F0] rounded-md px-3 py-1 text-sm focus:border-[#E2E8F0]-focus focus:ring-2 focus:ring-primary-500 focus:ring-offset-2"
                        >
                    ...
                    {activeChart === 'performance' && (
                        <>
                            <div className="flex items-center space-x-2">
                                ...
                                <span className="text-sm text-[#738192]">Listings</span>
                            </div>
                            <div className="flex items-center space-x-2">
                                <div className="w-3 h-3 bg-[#0EA5E9] rounded-full"></div>
                                <span className="text-sm text-[#738192]">Leads</span>
                            </div>
                            <div className="flex items-center space-x-2">
                                <div className="w-3 h-3 bg-[#059669] rounded-full"></div>
                                <span className="text-sm text-[#738192]">Sales</span>
                            </div>
                        </>
                    )}
                    {activeChart === 'revenue' && (
                        <div className="flex items-center space-x-2">
                            ...
                            <span className="text-sm text-[#738192]">Monthly Revenue</span>
                        </div>
                    )}
                </div>
            </div>

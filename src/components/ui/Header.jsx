...
const Header = () => {
    ...
    return (
        <header className="fixed top-0 left-0 right-0 bg-[#FFFFFF] border-b border-[#E2E8F0] z-999">
                            ...
                            <div className="relative">
                                <div className="absolute inset-y-0 left-0 pl-3 flex items-center pointer-events-none">
                                    <Icon
                                        name="Search"
                                        size={20}
                                        className={`transition-colors duration-200 ${isSearchFocused ? 'text-primary' : 'text-secondary'
                                            }`}
                                    />
                                </div>
                                <input
                                    ...
                                    className="block w-full pl-10 pr-4 py-2 border border-[#E2E8F0] rounded-md 
                           focus:border-[#E2E8F0]-focus focus:ring-2 focus:ring-[#3B82F6]focus:ring-offset-2 
                           transition-all duration-200 ease-out bg-background text-text-[#2563EB]
                    ...
                    <nav className="hidden md:flex items-center space-x-6">
                        {navigationItems.map((item) => (
                           ...
                           ${isActiveRoute(item.path)
                                            ? 'bg-[#DBEAFE] text-[#2563EB] border border-primary-500' : 'text-text-[#64748B]hover:text-text-[#64748B] hover:bg-[#F1F5F9]'
                                        }`}
                                >
                        ...
                        {user.isAuthenticated ? (
                            <div className="relative" ref={userMenuRef}>
                                <button
                                    ...
                                    className="flex items-center space-x-2 p-2 rounded-md hover:bg-[#F1F5F9] transition-all duration-200 ease-out micro-interaction"
                                ...
                                {/* User Dropdown */}
                                {isUserMenuOpen && (
                                    <div className="absolute right-0 mt-2 w-56 bg-[#FFFFFF] rounded-md shadow-elevation-3 
                                border  z-999">
                                        <div className="px-4 py-3 border-b border-[#E2E8F0]">
                                            <p className="text-sm font-medium text-text-[#2563EB]">{user.name}</p>
                                            <p className="text-xs text-text-[#64748B] capitalize">{user.role}</p>
                                        </div>
                                        <div className="py-1 ">
                                            {userMenuItems.map((item) => (
                                                <div key={item.label}>
                                                    {item.path ? (
                                                        <Link
                                                            to={item.path}
                                                            className="flex items-center space-x-3 px-4 py-2 text-sm text-text-[#64748B] hover:text-text-[#2563EB] hover:bg-[#DBEAFE] transition-colors duration-200"
                                                        >
                                                        ...
                                                        <button
                                                            ...
                                                            className="w-full flex items-center space-x-3 px-4 py-2 text-sm text-text-secondary hover:text-text-[#2563EB] hover:bg-[#F1F5F9] transition-colors duration-200"
                                                        >
                                                            <Icon name={item.icon} size={16} />
                                                            <span>{item.label}</span>
                                                        </button>
                    ...
                    <form onSubmit={handleSearchSubmit}>
                        <div className="relative">
                            ...
                            <input
                                ...
                                className="block w-full pl-10 pr-4 py-2 border border-[#E2E8F0] rounded-md 
                         focus:border-[#E2E8F0]-focus focus:ring-2 focus:ring-primary-500 
            ...
            {isMobileMenuOpen && (
                <div
                    ref={mobileMenuRef}
                    className="md:hidden bg-[#FFFFFF] border-t border-[#E2E8F0] z-mobile-menu"
                >
